...
class ConfigCog(commands.Cog):
    ...
    async def server_autocomplete(self, interaction: discord.Interaction, current: str):
        ...
        for srv, data in servers_info.items():
            if current.lower() in srv.lower():
                ...
                if is_enabled and is_featured:
                    full_name = f"{display_name} ({lbl_featured})"[:100]
                    choix_or.append(app_commands.Choice(name=full_name, value=srv))
                elif is_enabled:
    ...
    async def c_setup(self, interaction: discord.Interaction, language: app_commands.Choice[str], server: str):
        ...
        is_supported = srv_data.get("enabled", False)
        is_featured = srv_data.get("featured", False)
        ...
        pastille = "🌟" if is_featured else "🟢"

        embed.add_field(name=lbl_lang, value=language.name, inline=False)
        embed.add_field(name=lbl_srv, value=f"`{serveur_upper}` {pastille}", inline=False)
