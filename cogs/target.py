...
class CustomMinMaxModal (discord .ui .Modal ):
    def __init__ (self ,title ,ph_min ,ph_max ,wizard ,key_min ,key_max ,next_step ,langue ="en"):
        ...
        label =lbl_min ,placeholder =ph_min ,required =True ,default =str (wizard .config .get (key_min ,-1 ))
        )
        self .val_max =discord .ui .TextInput (
        label =lbl_max ,placeholder =ph_max ,required =True ,default =str (wizard .config .get (key_max ,-1 ))
        )
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        try :
            ...
            err =t (
            self .langue ,"wizard_err_nan",defaut ="❌ Invalid values. Please enter only numbers (-1 to ignore)."
            )
...
class ToggleButton (discord .ui .Button ):
    ...
    emoji_on =None ,
    emoji_off =None ,
    style_on =discord .ButtonStyle .success ,
    ...
    row =0 ,
        ...
        emoji =emoji_on if is_on else emoji_off ,
    ...
    async def callback (self ,interaction :discord .Interaction ):
        ...
        self .emoji =self .emoji_on if is_on else self .emoji_off 
...
class CycleButton (discord .ui .Button ):
    def __init__ (self ,key ,wizard ,states ,row =0 ):
        ...
        super ().__init__ (
        label =current_state ["label"],emoji =current_state .get ("emoji"),style =current_state ["style"],row =row 
        )
    ...
    async def callback (self ,interaction :discord .Interaction ):
        ...
        self .emoji =next_state .get ("emoji")
...
class TargetWizard (discord .ui .View ):
    ...
    async def update_view (self ,interaction :discord .Interaction ,initial =False ,content =None ):
        ...
        if self .step ==1 :
            embed .title =t (self .langue ,"wizard_s1_title",defaut ="{e_std_balance_scale} Step 1/7 : Absolute Level")
            embed .description =t (
            self .langue ,
            "wizard_s1_desc",
            defaut =(
            ...
            async def cb_custom (i ):
                ...
                CustomMinMaxModal (titre ,ph1 ,ph2 ,self ,"lvl_min","lvl_max",2 ,self .langue )
                )
            ...
            async def cb_custom (i ):
                ...
                await i .response .send_modal (
                CustomMinMaxModal (titre ,"Ex: 1000000","Ex: 5000000",self ,"pp_min","pp_max",3 ,self .langue )
                )
            ...
            async def cb_custom (i ):
                ...
                await i .response .send_modal (
                CustomMinMaxModal (titre ,"Ex: 10","Ex: 150",self ,"dist_min","dist_max",4 ,self .langue )
                )
            ...
            async def cb_custom (i ):
                ...
                await i .response .send_modal (
                CustomMinMaxModal (titre ,"Ex: 1000","Ex: 99999",self ,"honor_min","honor_max",5 ,self .langue )
                )
            ...
            async def cb_custom (i ):
                ...
                await i .response .send_modal (
                CustomMinMaxModal (titre ,"Ex: 10000","Ex: 500000",self ,"glory_min","glory_max",6 ,self .langue )
                )
            ...
            async def cb_custom (i ):
                ...
                await i .response .send_modal (
                CustomMinMaxModal (titre ,"Ex: 50000","Ex: 9999999",self ,"loot_min","loot_max",7 ,self .langue )
                )
            ...
            embed .title =t (self .langue ,"wizard_s7_title",defaut ="{e_parameters} Step 7/7 : Advanced Filters")
    ...
    def generate_summary_embed (self ):
        ...
        if act =="active":
            txt_act =t (self .langue ,"wizard_val_act",defaut ="{e_std_green_circle} Active only")
        elif act =="inactive":
            txt_act =t (self .langue ,"wizard_val_inact",defaut ="{e_std_sleeping_face} Ruins only")
        else :
            txt_act =t (self .langue ,"wizard_val_all",defaut ="♾️ Active & Ruins")
...
class CiblePaginationView (discord .ui .View ):
    ...
    async def btn_rerun (self ,interaction :discord .Interaction ,button :discord .ui .Button ):
        ...
        msg =t (self .langue ,"target_lbl_calc_rerun",defaut ="Launching a new radar scan...")
        await interaction .response .edit_message (content =msg ,embed =None ,view =self )

        await self .cog ._execute_cible (
        interaction ,self .attacker ,self .sort_by ,self .target_alliance ,message_to_edit =interaction .message 
        )
...
class TargetCog (commands .Cog ):
    ...
    async def target_search (self ,interaction :discord .Interaction ,origin_player :str ,target_alliance :str =None ):
        ...
        if not user_rules :
            ...
            langue ,
            "target_err_no_setup",
            defaut ="Stop! You must set up your search engine filters first:",
            )
    ...
    async def _execute_cible (
    self ,
    interaction :discord .Interaction ,
    origin_player :str ,
    sort_by :str ="aleatoire",
    target_alliance :str =None ,
    message_to_edit =None ,
        ...
        if message_to_edit :
            await message_to_edit .edit (content =None ,embed =loading_embed ,view =None )
        else :
            await interaction .edit_original_response (content =None ,embed =loading_embed ,view =None )
            ...
            async with session_active .get (coords_url ,headers =headers ,timeout =5 )as r :
                if r .status ==200 :
                    ...
                    if isinstance (c_data ,list ):
                        for c in c_data :
                            if (
                            str (c .get ("kingdomId",c .get ("kingdom_id","X")))=="0"
                            and str (c .get ("type",c .get ("castle_type","X")))=="1"
                            ):
        ...
        if a_coords ["x"]is None or a_coords ["y"]is None :
            err_msg =t (
            langue ,
            "target_err_atk_not_found",
            atk =origin_player ,
            defaut ="{e_error} Cannot locate player on the map to calculate distances.",
            )
            if message_to_edit :
                await message_to_edit .edit (content =err_msg ,embed =None ,view =None )
            else :
        ...
        if act_filter =="active":
            params ["inactiveFilter"]=0 
        elif act_filter =="inactive":
            params ["inactiveFilter"]=1 
        ...
        if target_alliance :
            params ["alliance"]=target_alliance 

        mots_interdits_mur ,alliances_mur_alerte =(
        ["repos","deuil","hospitalisé","rest","hospital","mourning","ruhe","krankenhaus"],
        [],
        )
        try :
        ...
        for page in pages_to_visit :
            ...
            try :
                async with session_active .get (api_url ,headers =headers ,timeout =10 )as r :
                    ...
                    players_list =(
                    data .get ("players",data .get ("items",data .get ("data",data )))
                    if isinstance (data ,dict )
                    else data 
                    )
                    ...
                    async def process_player (p ):
                        ...
                        if not t_name or t_name .lower ()==a_name_real .lower ():
                            return None 
                        ...
                        if config ["lvl_min"]!=-1 and t_abs_lvl <config ["lvl_min"]:
                            return None 
                        if config ["lvl_max"]!=-1 and t_abs_lvl >config ["lvl_max"]:
                            return None 

                        t_pp =int (p .get ("might_current",0 ))
                        if config ["pp_min"]!=-1 and t_pp <config ["pp_min"]:
                            return None 
                        if config ["pp_max"]!=-1 and t_pp >config ["pp_max"]:
                            return None 

                        t_honor =int (p .get ("honor",0 ))
                        if config ["honor_min"]!=-1 and t_honor <config ["honor_min"]:
                            return None 
                        if config ["honor_max"]!=-1 and t_honor >config ["honor_max"]:
                            return None 

                        t_loot =int (p .get ("loot_current",0 ))
                        if config ["loot_min"]!=-1 and t_loot <config ["loot_min"]:
                            return None 
                        if config ["loot_max"]!=-1 and t_loot >config ["loot_max"]:
                            return None 

                        t_glory =int (p .get ("current_fame",p .get ("highest_fame",0 )))
                        if config ["glory_min"]!=-1 and t_glory <config ["glory_min"]:
                            return None 
                        if config ["glory_max"]!=-1 and t_glory >config ["glory_max"]:
                            return None 

                        tx ,ty ,dist ="???","???",9999 
                        try :
                            async with session_active .get (
                            f"https://api.gge-tracker.com/api/v1/castle/search/{urllib.parse.quote(t_name)}",
                            headers =headers ,
                            timeout =5 ,
                            )as res :
                                if res .status ==200 :
                                    ...
                                    for c in c_data :
                                        ...
                                        and str (c .get ("type",c .get ("castle_type","X")))=="1"
                                            ...
                                            if tx is not None and ty is not None :
                                                dist =math .sqrt (
                                                (float (tx )-float (a_coords ["x"]))**2 
                                                +(float (ty )-float (a_coords ["y"]))**2 
                                                )
                                            break 
                        except :
                            pass 
                        ...
                        if (
                        tx =="???"
                        or (config ["dist_min"]!=-1 and dist <config ["dist_min"])
                        or (config ["dist_max"]!=-1 and dist >config ["dist_max"])
                        ):
                            return None 

                        alli_clean ="".join (c for c in t_alli .lower ()if c .isalnum ())
                        has_wall_warning =alli_clean in alliances_mur_alerte or any (
                        mot in str (t_alli ).lower ()for mot in mots_interdits_mur 
                        )
        ...
        if not final_targets :
            ...
            if message_to_edit :
                await message_to_edit .edit (content =empty_msg ,embed =None ,view =None )
            else :
        ...
        target_a_str =t (
        langue ,
        "target_cible_alli_target",
        a =target_alliance ,
        defaut =f" (Targeted Alliance: {target_alliance})",
        )
        ...
        for i in range (0 ,len (best_targets ),chunk_size ):
            ...
            a =a_name_real ,
            t =titre_alliance ,
            defaut =f"{{e_attaque}} Scan Results from {a_name_real}{titre_alliance}",
            )
            desc_emb =t (
            langue ,
            "target_res_desc",
            shown =len (best_targets ),
            total =len (final_targets ),
            defaut =f"{{e_icon_search}} **{len(best_targets)} targets displayed** (randomly selected from {len(final_targets)} valid targets found).\n━━━━━━━━━━━━━━━━━━━━━━━",
            )
            ...
            for j ,t_cnd in enumerate (chunk ):
                ...
                "target_dist_lieues",
                d =int (t_cnd ["dist"]),
                defaut =f"{int(t_cnd['dist'])} leagues",
                )
                ...
                if t_cnd ["wall_warning"]:
                    ...
                    langue ,
                    "target_warn_wall",
                    defaut ="\n{e_error} **CHECK WALL:** Sensitive alliance description!",
                    )
                    )
                if is_under_colombe :
                    warnings .append (
                    ...
                    langue ,
                    "target_warn_peace",
                    defaut ="\n{e_peace} **PLAYER UNDER DOVE: Protection active!**",
                    )
                    )
                ...
                name_cible =t (
                langue ,
                "target_cible_field_title",
                idx =index_global ,
                n =t_cnd ["name"],
                icon =target_icon ,
                defaut =f"{target_icon} Target #{index_global} : {t_cnd['name']}",
                )
                embed .add_field (name =name_cible ,value =description_cible ,inline =False )

            titre_page =t (
            langue ,
            "target_cible_footer_page",
            cur =page_num ,
            tot =nb_pages ,
            defaut =f"Page {page_num}/{nb_pages}",
            )
            val_spy =t (langue ,"target_cible_footer_spy",defaut ="{e_icon_name} **SPY REQUIRED** before attack.")
            embed .add_field (name =titre_page ,value =val_spy ,inline =False )

            await setup_embed_footer (embed ,interaction ,langue )
            embeds .append (embed )

        view =CiblePaginationView (
        self ,origin_player ,sort_by ,target_alliance ,embeds ,langue ,owner_id =interaction .user .id 
        )
